...
class SubscriptionControllerTest {
  ...
  void setUp() {
    ...
    controller =
        new SubscriptionController(
            userRepository, courseRepository, subscriptionRepository, eventProducer);
  }
  ...
  void subscribe_unexpectedError_returnsInternalServerError() {
    when(userRepository.findById(USER_ID)).thenThrow(new RuntimeException("DB connection lost"));
  ...
  /**
   * NB: ce test suppose que la vérification anti-doublon (recommandée dans le README) a été ajoutée
   * au controller via subscriptionRepository.findByUserIdAndCourseId(...). Si ce n'est pas encore
   * fait, ce test échouera — c'est volontaire, ça sert de rappel/garde-fou.
   */
  @Test
